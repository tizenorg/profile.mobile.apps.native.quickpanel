...
static Evas_Object *_activenoti_create_icon(Evas_Object *parent, notification_h noti)
{
	...
	if (ret == NOTIFICATION_ERROR_NONE && tmp != NULL) {
		...
		if (ret == NOTIFICATION_ERROR_NONE && tmp != NULL) {
			...
			if (!icon_path) {
				snprintf(default_icon_path, sizeof(default_icon_path), "%simages/%s", app_get_resource_path(), QP_DEFAULT_ICON_NAME);
				DBG("default_icon_path :  %s", default_icon_path);
				icon_path = strdup(default_icon_path);
			}
...
static void _activenoti_create_activenoti(void)
{
	...
	ret = elm_layout_file_set(s_info.layout, util_get_res_file_path(ACTIVENOTI_EDJ), "headsup/base");
